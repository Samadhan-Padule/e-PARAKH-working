...
def _infer_product_name(lines, label_indexes):
    for index, field, inline_value in label_indexes:
        if field == "product_name" and inline_value:
            return inline_value
    for line in lines:
        if line == "Kaju Katli (Classic)":
            return line
    for line in lines:
        if line and not any(token in line.lower() for token in ("ingredients", "nutrition", "manufacturer", "fssai", "mrp", "batch", "net", "customer", "product of")) and not any(char.isdigit() for char in line) and len(line) > 4:
            return line
    return None
